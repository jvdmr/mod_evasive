...
static int ntt_node_is_outdated(const struct ntt_node *node, time_t timestamp) {
    return timestamp - node->timestamp >= 6 * 60 * 60; /* 6 hours */
}

/* Copy a node into the tree; only used during tree growth */

static void ntt_grow_copy(struct ntt *ntt, struct ntt_node *node, time_t timestamp) {
    size_t hash_code;
...
static int ntt_grow(struct ntt *ntt, time_t timestamp) {
    struct ntt tmp_ntt;
