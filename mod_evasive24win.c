...
/*
   ...
   Windows port by Gregg Smith <greg apachehaus com>
   Apache 2.4 compatibility and many improvements by Joris Vandermeersch <joris.vdmr@gmail.com>
...
#define DEFAULT_LOG_DIR         "/temp"  // Default temp directory
...
static int access_checker(request_rec *r) 
{
    ...
    if (cfg->enabled && r->prev == NULL && r->main == NULL && cfg->hit_list != NULL) {
        ...
        if (ret == cfg->http_reply) {
            ...
            if (file != NULL) {
                ...
                fprintf(file, "%s\nPID: %ld\n", evasive_timestamp, getpid());
                fprintf(file, "Blacklisting address %s: possible DoS attack.\n", r->useragent_ip);
                fclose(file);

                /* This should be possible in Windows, consider it Experimental */
    ...
    /* Give a hoot, don't pollute the Apache's error log ok? 
     *
     *    if (ret == cfg->http_reply
